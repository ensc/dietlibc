...
#ifndef __NO_CODE  /* We want the text table, not only __SYS_NERR */
...
const int  sys_nerr = sizeof (sys_errlist) / sizeof(*sys_errlist) - 1;

#endif /* __NO_CODE */


/*
 *  Some code can be coded shorter if this constant can be coded immediately
 *  instead of indirect:
 *
 *     perror(), strerror(), __unified_syscall()
 *
 *  The disadvantage is that we must also care for __SYS_NERR, because in
 *  C can't determine the size of a array outside the module.
 */
