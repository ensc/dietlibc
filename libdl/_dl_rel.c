...
static void exit_now(void) {
  DEBUG(printf("exit_now: symbol not found\n");)
  _exit(213);
}
...
unsigned long do_rel(struct _dl_handle * tmp_dl, unsigned long off)
{
  ...
  DEBUG(printf("do_rel %08lx %08lx\n",(long)tmp_dl,off);)
  DEBUG(printf("do_rel %08lx+%lx\n",(long)tmp_dl->plt_rel,off);)
  DEBUG(printf("do_rel @ %08lx with type %d -> %d\n",(long)tmp->r_offset,ELF32_R_TYPE(tmp->r_info),sym);)
  DEBUG(printf("do_rel sym @ %08lx\n",(long)tmp_dl->dyn_sym_tab);)
